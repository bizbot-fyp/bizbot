...
async def receive_from_make(
    ...
    # FIX: Store all post attributes in local variables BEFORE the next commit!
    # The next db.commit() will "expire" the post object in SQLAlchemy's async memory
    post_id_val = post.id
    ...
    notification = SocialMediaNotification(
        ...
        message=f"📝 New {platform_val} post generated by AI needs review",
    ...
    # We now safely use our local variables instead of the expired 'post' object
